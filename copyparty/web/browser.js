...
var Ls = {
	...
	"chi": {
		...
		"ot_mp": "媒体播放器选项",
		"ot_cfg": "配置选项",
		"ot_u2i": "up2k：上传文件（如果你有写入权限），或切换到搜索模式以查看文件是否存在于服务器上,\n\n上传是可恢复的，多线程的，保留文件时间戳，但比 [🎈]（基础上传器）占用 更多的CPU，\n\n上传过程中，此图标会变成进度指示器！",
		"ot_u2w": "up2k：带有恢复支持的文件上传（关闭浏览器后，重新上传相同文件）\n\n多线程的，文件时间戳得以保留，但比 [🎈] （基础上传器）使用更多CPU，\n\n上传过程中，这个图标会变成进度指示器！",
		...
		"cut_mt": "使用多线程加速文件哈希\n\n这使用 Web Worker 并且需要更多内存（额外最多 512 MiB）\n\n30% 更快的 https，4.5 倍更快的 http，和 5.3 倍更快的 Android 手机",
		...
		"mt_drc": "启用动态范围压缩器（音量平滑器 / 限幅器）；还会启用均衡器以平衡音频，因此如果你不想要它，请将均衡器字段除了 '宽度' 外的所有字段设置为 0\n\n降低 THRESHOLD dB 以上的音频的音量；每超过 THRESHOLD dB 的 RATIO 会有 1 dB 输出，所以默认值 tresh -24 和 ratio 12 意味着它的音量不应超过 -22 dB，可以安全地将均衡器增益提高到 0.8，甚至在 ATK 0 和 RLS 如 90 的情况下提高到 1.8（仅在 Firefox 中有效；其他浏览器中 RLS 最大为 1）\n（见维基百科，他们解释得更好）",
