...
Ls.chi = {
	...
	"cut_turbo": "不作死就不会死，谨慎使用，后果自负！$N$N如果你上传大量文件到一半，由于某些原因不得不重新启动，$N然后想立刻开始继续上传，则可开启此选项$N$N开启后，哈希校验将被简单的 <em>“服务器上的文件大小是否相同？”</em> 取代，$N因此内容不同但大小一致的文件将不会被上传$N$N你应该在上传完成后关闭此选项，$N然后重新“上传”同一批文件以供客户端校验\">超频", // turbo button 指 386 时代机箱上的降频按钮。此处 YOLO 选译为“作死”。

	"cut_datechk": "只在启用超频时有效$N$N略微降低作死程度：检查服务器上的文件时间戳是否与你的一致$N$N<em>理论上</em> 应该能发现大部分未上传完成/损坏的文件，$N但不能取代之后禁用“超频”再次校验\">戳检",
	...
	"u_s404": "在服务器上未找到",
	"u_expl": "解释",
	"u_maxconn": "大多数浏览器限制为 6，但 Firefox 允许你在 <code>about:config</code> 中配置 <code>connections-per-server</code> 来提高此限制",
